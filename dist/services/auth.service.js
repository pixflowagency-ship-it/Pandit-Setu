...
import { createOtp, getOtpExpirySeconds, normalizePhone, verifyOtp, } from "./otp.service.js";
...
export async function verifyOtpAndLogin(phone, otp, name) {
    const normalizedPhone = normalizePhone(phone);
    const isValid = verifyOtp(normalizedPhone, otp);
    if (!isValid) {
        throw new AppError(401, "INVALID_OTP", "The OTP provided is invalid or has expired");
    }
    ...
    if (!user) {
        if (!name?.trim()) {
            throw new AppError(400, "NAME_REQUIRED", "Name is required for new user registration");
        }
